    ...
    public sealed class CommentRepository : Repository<T_Comment>, ICommentRepository
    {
        ...
        public async Task<T_Comment?> GetWithInnerTreeByIdAsync(Guid id)
        {
            var result = await DbSet.FromSqlRaw(
                @"WITH tree (id, content, creationTime, articleId, authorId, parentCommentId, below) AS (
                        SELECT comment.id, content, creationTime, articleId, authorId, parentCommentId, 0
                        FROM Comments as comment
		                LEFT JOIN Users as u On AuthorId = u.Id
		                WHERE comment.Id = {0}
                        UNION ALL
                        SELECT c.id, c.content, c.creationTime, c.articleId, c.authorId, c.parentCommentId, t.below + 1
                        FROM dbo.Comments c
                        INNER JOIN tree t
