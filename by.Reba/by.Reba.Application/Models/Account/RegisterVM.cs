...
namespace by.Reba.Application.Models.Account
{
    public class RegisterVM
    {
        ...
        [Remote(controller: "Account", action: "VerifyNicknameAsync", HttpMethod = WebRequestMethods.Http.Post)]
        [MaxLength(30, ErrorMessage = "Никнейм должен быть не более чем 30 символов")]
        public string Nickname { get; set; }

        [Required(ErrorMessage = "Поле обязательно для заполнения")]
        [Remote(controller: "Account", action: "VerifyEmailAsync", HttpMethod = WebRequestMethods.Http.Post)]
        [EmailAddress]
